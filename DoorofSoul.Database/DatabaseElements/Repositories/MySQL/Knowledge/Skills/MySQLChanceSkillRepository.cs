...
namespace DoorofSoul.Database.DatabaseElements.Repositories.MySQL.Knowledge.Skills
{
    public class MySQLChanceSkillRepository : ChanceSkillRepository
    {
        ...
        {
            string sqlString = @"INSERT INTO Knowledge_Skills_ChanceSkills 
        ...
        {
            string sqlString = @"DELETE FROM Knowledge_Skills_ChanceSkills 
            ...
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                ...
                {
                    DataBase.Instance.Log.ErrorFormat("MySQLChanceSkillRepository Delete Skill Error SkillID: {0}", skillID);
                }
            }
        }
        ...
        public override Skill Find(int skillID)
        {
            string sqlString = @"SELECT  
                ...
                from Knowledge_Skills_ChanceSkills WHERE SkillID = @skillID;";
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                command.Parameters.AddWithValue("@skillID", skillID);
                using (MySqlDataReader reader = command.ExecuteReader())
        ...
        public override List<Skill> List()
        {
            string sqlString = @"SELECT  
                ...
                from Knowledge_Skills_ChanceSkills;";
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                using (MySqlDataReader reader = command.ExecuteReader())
                {
        ...
        {
            string sqlString = @"UPDATE Knowledge_Skills_ChanceSkills SET 
            ...
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                ...
                if (command.ExecuteNonQuery() <= 0)
                {
                    DataBase.Instance.Log.ErrorFormat("MySQLChanceSkillRepository Save Skill Error WoridID: {0}", skill.SkillID);
                }
            }
        }
    }
