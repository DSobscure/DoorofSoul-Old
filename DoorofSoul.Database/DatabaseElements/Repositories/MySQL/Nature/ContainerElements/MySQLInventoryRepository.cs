using DoorofSoul.Database.DatabaseElements.Repositories.Nature.ContainerElements;
using DoorofSoul.Library.General.NatureComponents.ContainerElements;
using MySql.Data.MySqlClient;

namespace DoorofSoul.Database.DatabaseElements.Repositories.MySQL.Nature.ContainerElements
{
    public class MySQLInventoryRepository : InventoryRepository
    {
        public override Inventory Create(int containerID, int capacity)
        {
            string sqlString = @"INSERT INTO Nature_ContainerElements_Inventories 
        ...
        {
            string sqlString = @"DELETE FROM Nature_ContainerElements_Inventories 
        ...
        public override Inventory Find(int inventoryID)
        {
            string sqlString = @"SELECT  
                ...
                from Nature_ContainerElements_Inventories WHERE InventoryID = @inventoryID;";
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                command.Parameters.AddWithValue("@inventoryID", inventoryID);
                using (MySqlDataReader reader = command.ExecuteReader())
        ...
        public override Inventory FindByContainerID(int containerID)
        {
            string sqlString = @"SELECT  
                ...
                from Nature_ContainerElements_Inventories WHERE ContainerID = @containerID;";
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                command.Parameters.AddWithValue("@containerID", containerID);
                using (MySqlDataReader reader = command.ExecuteReader())
        ...
        {
            string sqlString = @"UPDATE Nature_ContainerElements_Inventories SET 
