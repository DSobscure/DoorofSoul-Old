...
{
    public class MySQLAlchemySkillRepository : AlchemySkillRepository
    {
        ...
        {
            string sqlString = @"INSERT INTO Knowledge_Skills_AlchemySkills 
        ...
        {
            string sqlString = @"DELETE FROM Knowledge_Skills_AlchemySkills 
            ...
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                ...
                {
                    DataBase.Instance.Log.ErrorFormat("MySQLAlchemySkillRepository Delete Skill Error SkillID: {0}", skillID);
                }
            }
        }
        ...
        public override Skill Find(int skillID)
        {
            string sqlString = @"SELECT  
                ...
                from Knowledge_Skills_AlchemySkills WHERE SkillID = @skillID;";
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                command.Parameters.AddWithValue("@skillID", skillID);
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        return new Skill(skillID, HeptagramSystemTypeCode.Alchemy, mediaTypeCode, skillName, basicLifePointCost, basicEnergyPointCost, basicCorePointCost, basicSpiritPointCost);
                    }
                    else
                    {
                        return null;
        ...
        public override List<Skill> List()
        {
            string sqlString = @"SELECT  
                ...
                from Knowledge_Skills_AlchemySkills;";
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        skills.Add(new Skill(skillID, HeptagramSystemTypeCode.Alchemy, mediaTypeCode, skillName, basicLifePointCost, basicEnergyPointCost, basicCorePointCost, basicSpiritPointCost));
                    }
                    return skills;
                }
            }
        ...
        {
            string sqlString = @"UPDATE Knowledge_Skills_AlchemySkills SET 
            ...
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                ...
                if (command.ExecuteNonQuery() <= 0)
                {
                    DataBase.Instance.Log.ErrorFormat("MySQLAlchemySkillRepository Save Skill Error WoridID: {0}", skill.SkillID);
                }
            }
        }
    }
