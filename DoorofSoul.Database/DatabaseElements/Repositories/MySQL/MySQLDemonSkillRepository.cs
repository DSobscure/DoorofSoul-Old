            ...
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        return new Skill(skillID, HeptagramSystemTypeCode.Demon, skillName, basicLifePointCost, basicEnergyPointCost, basicCorePointCost, basicSpiritPointCost);
                    }
                    else
                    {
                        return null;
            ...
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        skills.Add(new Skill(skillID, HeptagramSystemTypeCode.Demon, skillName, basicLifePointCost, basicEnergyPointCost, basicCorePointCost, basicSpiritPointCost));
                    }
                    return skills;
                }
            }
