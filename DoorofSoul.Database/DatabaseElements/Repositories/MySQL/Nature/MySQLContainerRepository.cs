...
namespace DoorofSoul.Database.DatabaseElements.Repositories.MySQL.Nature
{
    public class MySQLContainerRepository : ContainerRepository
    {
        ...
        {
            Entity entity = DataBase.Instance.RepositoryManager.NatureList.EntityRepository.Create(entityName, locatedSceneID, spaceProperties);
            ...
            if (entity != null)
            {
                ...
                Inventory inventory = DataBase.Instance.RepositoryManager.NatureList.ContainerElementsList.InventoryRepository.Create(containerID, 40);
            ...
            using (MySqlCommand command = new MySqlCommand(sqlString, DataBase.Instance.Connection as MySqlConnection))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        Entity entity = DataBase.Instance.RepositoryManager.NatureList.EntityRepository.Find(entityID);
                        Inventory inventory = DataBase.Instance.RepositoryManager.NatureList.ContainerElementsList.InventoryRepository.FindByContainerID(containerID);
